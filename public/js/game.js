...
function startMenu() {
	...
	script.src = './js/words/' + maxBlock + '.js';
...
function gameStart(words_string = "") {
	...
	if (words_string.length > 0) {
		...
		var wordType = (level == 'beginner') ? beginner : ((level == 'intermediate') ? intermediate : ((level == 'advanced') ? advanced : ((level == 'godmode') ? fullList : custom)));
	}
	...
	difficultySelect.innerText = level.charAt(0).toUpperCase() + level.slice(1);
	...
	notification.innerText = 'بدء التخمين!'
...
function quitQlick() {
	if (gameFin == 0) {
		notification = 'The word was ' + chosenWord + ' Click to play again';
		currentStreak = 0;
...
function submitWord(wordRow) {
	if (nextRowBlock > 0 && nextRowBlock % maxBlock == 0) {
		...
		remNotification = 0;
		document.getElementById('notification').innerText = 'يجب عليك ادخال ' + maxBlock + ' محارف';
	}
}
